 ...
 * 
 * By default cache is used in order make the API faster but it's disabled 5min
 * after and before any event in order to give real time updates.
 * 
...
class EconomicCalendarAPI
{
    ...
    private $minGapFromNextEvent = 300; // Cache is disabled 5 min after/before an event
    ...
    public function fetchEvents()
    {
        ...
        if ($this->shouldUseCache()) {
            header("API-Info-Using-Cache: true");
            $this->serveFromCache();
        } else {
            header("API-Info-Using-Cache: false");
    ...
    private function shouldUseCache()
    {
        ...
        return ($cacheTime - time()) > $this->minGapFromNextEvent;
    }

    private function shouldCache()
    {
        $cacheTime = $this->extractTimestampFromFilePath($this->cacheFile);
        return (time() - $cacheTime) > $this->minGapFromNextEvent; // Just caches after minutes of gap are passed
    }
